...
class RafflesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create raffle" do
    assert_difference('Raffle.count') do
      post raffles_url, params: { raffle: { name: @raffle.name, number: @raffle.number, status: @raffle.status } }
    end
  ...
  test "should update raffle" do
    patch raffle_url(@raffle), params: { raffle: { name: @raffle.name, number: @raffle.number, status: @raffle.status } }
